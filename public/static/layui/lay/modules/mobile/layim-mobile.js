...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  //群直播状态
  LAYIM.prototype.liveState = function(data){
    return liveState(data), this;
  };  
  //删除历史记录
  LAYIM.prototype.removeHistory = function(data){
    return removeHistory(data), this;
  };
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-id="{{data.id}}" data-index="'+ (options.index ? '{{'+ options.index +'}}' : (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}}') +'" class=" '+(options.type === 'history' ? 'list-li' : '')+' layim-'+ (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}} {{ data.status === "offline" ? "layim-list-gray" : "" }}"><div class="left" style="width: 100%;"><div><img src="{{data.avatar}}"></div><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></div>'+(options.type === 'history' ? '<div layim-event="removeHistory" class="swipe-delete-btn btn">删除</div>' : '')+'</li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var comTpl = function(tpl, anim, back){
    return ['<div class="layim-panel'+ (anim ? ' layui-m-anim-left' : '') +'">'
      ,'<div class="layim-title" style="background-color: {{d.base.chatTitleColor}};">'
        ,'<p>'
          ,(back ? '<i class="layui-icon layim-chat-back" {{# if(d.data && d.data.type === "group"){ }} id="layim-chat-group-back" {{# } else{ }} id="layim-chat-back" {{# } }} layim-event="back">&#xe603;</i>': '') 
          ,'{{ d.title || d.base.title }}<span class="layim-chat-status"></span>'
          ,'{{# if(d.data){ }}'
            ,'{{# if(d.data.type === "group"){ }}'
              ,'<i class="layui-icon layim-chat-detail" id="layim-chat-detail" layim-event="detail">&#xe613;</i>'
            ,'{{# } }}'
          ,'{{# } }}'
        ,'</p>'
      ,'</div>'
  ...
  //聊天主模板
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}} layim-chat-{{d.data.type}}{{d.data.id}}">'
    ,'{{# if(d.data.type == "group"){ }}'
    ,'<div id="videodom" style="width:100%; height:auto;">'
    ,'</div>'
    // ,'<div class="layim-chat-main" style="top:250px">'
    ,'{{# }; }}'
    // ,'{{# if(d.data.type != "group"){ }}'
    ...
    ,'<img layim-event="hiddenBigPic" id="demo" src="" alt="" class="laykefu-bigimg" style="display:none;">'
    ,'<div>'
    ,'<div class="laykefu-mask" style="display: none;"></div>'
  ,'</div>'].join('');
  ...
  layui.data.content = function(content){
    ...
    .replace(/img\[([^\s]+?)\]/g, function(img){  //转义图片
      return '<img layim-event="showBigPic" class="layui-layim-photos" width="120px" src="' + img.replace(/(^img\[)|(\]$)/g, '') + '">';
    })
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if(data.type === 'friend'){
      ...
      layui.each(cache.group, function(index, item){
        if(item.id == data.id || data.id == 'admin'){
          data.type = 'group';
  ...
  //群直播状态
  var liveState = function(data){
    var data = '{"groupid":1,"state":-11}';
     data = $.parseJSON( test )
    var group = cache['group'];
    layui.each(group, function(index, item){
      if(data.groupid == item.id){
        cache['group'][index].state = data.state; 
        var groupInfo = cache['group'][index]; 
        popchat(data, true);        
      }
    });  
  };

  var removeHistory = function (data){
    $(".layim-list-history").find('.layim-'+data.type+data.id).remove();
    var mine = layui.data('layim-mobile')[cache.mine.id];
    var history = mine.history;
    $.each(history,function(n,value) {
        if (n == (data.type+data.id)){
            delete history[n];
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      ...
      touch_method = othis.attr('touch_method');
      var list = othis.attr('data-list') || othis.index(), data = {};
      if(type === 'friend'){
        var friend = cache[type][index].list;
        for (var i = 0; i < friend.length; i++) {
          if (friend[i].id == othis.data('id')) {
            data = friend[i];
          }
        }
      ...
      data.touch_method = touch_method;
      popchat(data, true);
    ...
    ,addGroup: function (othis){
      ...
      layer.open({
        ...
        ,btn: ['确认', '取消']
        ,yes:function(index){
          $.get("/index/index/apply?groupid="+groupid,function(res){
            layer.open({
              content: res.msg
              ,skin: 'msg'
              ,time: 2 //2秒后自动关闭
    ...
    ,showBigPic: function (othis){
      $(".laykefu-mask").css('display','block');
      $(".laykefu-bigimg").attr("src", $(this).attr("src")).css('display','block');
    }
    ,hiddenBigPic: function (othis){
      $(".laykefu-bigimg").css('display','none');
      $(".laykefu-mask").css('display','none');
    }
    ,swipe: function (){
    ...
    ,removeHistory: function(othis){
      var index = othis.parent().data('index');
      var local = layui.data('layim-mobile')[cache.mine.id] || {};
      var data = (local.history || {})[index] || {};
      removeHistory(data);
    }      
    //查看用户详情
    ,userInfo: function(othis){
      var data = {};
      var thatChat = thisChat();
      data.userid = othis.data('id');
      data.groupid = othis.data('groupid');
      data.groupname = othis.data('groupname');
      layui.each(call.userInfo, function(index, item){
        item && item(data);
      });
    }      
    //设置面板
    ,setting: function(othis){
      var thatChat = thisChat();
      layui.each(call.setting, function(index, item){
        item && item(thatChat.data);
      });
    }     
    //设置昵称
    ,setname: function(othis){
      var thatChat = thisChat();
      layui.each(call.setname, function(index, item){
        item && item(thatChat.data);
      });
    }        
    //设置禁言
    ,gag: function(othis){
      var data = {};
      data.userid = othis.parent().data('id');
      data.groupid = othis.parent().data('groupid');
      layui.each(call.gag, function(index, item){
        item && item(data);
      });
    }       
    //取消禁言
    ,removeGag: function(othis){
      //底部对话框
      var data = {};
      data.userid = othis.parent().data('id');
      data.groupid = othis.parent().data('groupid');
    ...
    //确认禁言
    ,sure_gag: function(othis){
      var data = {};
      data.userid = othis.parent().data('id');
      data.groupid = othis.parent().data('groupid');
      data.gag = $(this).parent().parent().find("input[type='radio']:checked").val();
      layui.each(call.sure_gag, function(index, item){
        item && item(data);
      });
    }  
    //移出群
    ,removeGroup: function(othis){
      var data = {};
      data.userid = othis.parent().data('id');
      data.groupid = othis.parent().data('groupid');
      data.groupname = othis.parent().data('groupname');
      layui.each(call.removeGroup, function(index, item){
        item && item(data);
      }); 
    }   
    ,logout: function(othis){
      var userid = othis.data('index');
      layer.open({
        content: '确认退出登录？'
        ,className: 'layim-about'
        ,shadeClose: false
        ,btn: ['确认', '取消']
        ,yes:function(index){
          $.get("/index/index/logout?userid="+userid,function(res){
            if (res.code == 0) {
              layer.open({
                content: res.msg
                ,skin: 'msg'
